...
function Header({ hotelId }) {
  ...
  return (
    <header className="relative sticky top-0 z-50">
      ...
      >
        <Link to="/" className="z-50 text-2xl sm:text-2xl text-yellow-600 ">
          <img
            src="/assets/img/logo.png"
            className="w-16 inline-block rounded-lg"
            alt="logo"
          />
          <span className="hidden md:inline-block font-serif ml-8">
        ...
        <div className="flex">
          ...
          {state.auth.isAuthenticated && (
            <img
              ...
              src={
                auth && auth.user_image
                  ? `${process.env.REACT_APP_BASE_URL}/img/users/${auth.user_image}`
                  : "https://cedat.mak.ac.ug/wp-content/uploads/2019/08/blank-profile-male-254x300.png"
              }
            />
          )}
          <ul
              ...
              <>
                ...
                {!state.auth.is_admin ? (
                  <div
                    ...
                    style={
                      !dropDown ? { display: "none" } : { display: "block" }
                    }
                  >
                    <Link
                      to={`/user-profile/${id}`}
